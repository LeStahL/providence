...
#define NTIME 2
const float pos_B[2] = float[2](0.,96.);
const float pos_t[2] = float[2](0.,185.8065);
...
{
    return B<0 ? 0. : (B>=0. && B<2.) ? 0. : (B>=2. && B<6.) ? linmix(B, .25, -.5, 0.0, 1.0) : (B>=14. && B<15.5) ? linmix(B, .6667, -9.3333, 1.0, 0.1) : (B>=15.5 && B<16.) ? linmix(B, 2., -31., 0.1, 0.0) : 1.;
}
float fqmbace7sat_vol(float B)
{
    return B<0 ? 0. : (B>=0. && B<.5) ? linmix(B, 2., 0., 0.0, 1.0) : (B>=15. && B<16.) ? linmix(B, 1., -15., 1.0, 0.0) : (B>=80. && B<88.) ? 1.66 : (B>=88. && B<98.) ? linmix(B, .1, -8.8, 1.0, 0.0) : 1.;
}
float fett2_volume(float B)
{
    return B<0 ? 0. : (B>=0. && B<5.) ? linmix(B, .2, 0., 0.1, 1.0) : (B>=8. && B<11.) ? linmix(B, .3333, -2.6667, 1.0, 0.5) : (B>=11. && B<14.) ? linmix(B, .3333, -3.6667, 0.4, 1.0) : (B>=14. && B<15.5) ? linmix(B, .6667, -9.3333, 1.0, 0.1) : (B>=15.5 && B<16.) ? linmix(B, 2., -31., 0.1, 0.0) : (B>=91. && B<98.) ? linmix(B, .1429, -13., 1.0, 0.0) : 1.;
}
float hoboe_vol(float B)
{
    return B<0 ? 0. : (B>=0. && B<4.) ? linmix(B, .25, 0., 0.21, 1.5) : (B>=4. && B<15.) ? 1.6 : (B>=15. && B<15.5) ? linmix(B, 2., -30., 1.6, 0.1) : (B>=15.5 && B<16.) ? linmix(B, 2., -31., 0.1, 0.0) : 1.;
}
...
vec2 mainSynth(float time)
{
    ...
    if (time > 188.7097) return vec2(0);
...
env = theta(Bprog)*pow(1.-smstep(Boff-rel, Boff, B),3);
    ...
    float masterL = .18 * sidechain * s_atan(sL) + .8 * dL;
    float masterR = .18 * sidechain * s_atan(sR) + .8 * dR;
    return vec2(
        masterL,
        masterR);
}
